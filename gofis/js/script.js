...
const uploadXmlFile = e => {
  ...
  if (xmlFileNode.name === lastXmlFile)
    return;
  ...
  leitor.addEventListener("load", function () {
    ...
    Array.from(itens).forEach(function (element) {
      ...
      elements.push(parseFloat(element.querySelector('vUnCom').firstChild.nodeValue).toFixed(2));
      elements.push(parseInt(element.querySelector('qCom').firstChild.nodeValue));
